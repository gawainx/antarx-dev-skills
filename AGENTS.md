...
- 所有项目只要由 Git 管理且配置了 remote，完成工作的标准就必须包括：验证改动、提交改动、成功推送当前工作分支到对应远端，并核对远端已包含本次提交。仅本地提交或工作区干净不算完成，无需再次请求推送许可。
- 没有 upstream 不构成跳过推送的理由：根据已有配置和用户意图确定 remote 与目标分支，首次推送时建立 upstream；只有目标存在实质歧义时才询问。推送失败或受阻时继续排查；无法解决必须明确报告阻碍，不得声称工作已完成，也不得擅自强制推送。
