...
5. 根据功能领域推断文档分类。
6. 检查是否存在同前缀文档。
7. 若存在，先比较范围/日期/内容，并询问“追加还是覆盖”。
8. 先写设计文档，默认落盘到 `docs/design-docs/`。
9. 再写开发计划，默认落盘到 `docs/exec-plans/active/`，并在 `## 关联设计文档` 引用设计文档。
10. 严控范围，明确纳入范围与不纳入范围。

## 输出规则
- 语言跟随用户；默认简体中文。
- 文件命名默认使用 `{Feat}_{YYYYMMDD}.md`。
