...
**计划保存到：** `docs/plans/YYYY-MM-DD-<feature-name>.md`，但如果项目代码仓有其他规定，则需要严格遵守代码仓的规定
...
- 不要覆盖明确的项目约束或既有约定。
- 编写新特性计划时，先以当前需求、已批准设计和用户确认的成功标准为边界；历史文档中的约束不得直接作为新特性开发过程的约束边界，至少不能作为第一优先级。
- 如果当前新特性与历史需求、设计文档或旧计划中的约束存在冲突，必须先请求用户确认取舍；禁止未经确认就基于历史约束写入计划。
- 禁止将历史文档中的“非范围”“暂不实现”“不引入依赖”等负向边界带入当前计划，作为当前新特性的限制条件或评估证据。
...
- 计划必须组织为一个或多个阶段（`Stage #1`、`Stage #2`、...）。
...
- 范围边界必须来自当前需求或用户确认；历史约束只能作为背景，发现冲突先问用户再写计划
