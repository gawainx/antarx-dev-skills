...
3. 查找或创建滴答清单项目。
   ...
   - 精确或归一化匹配得到多个项目时，列出匹配项目的名称和 id，请用户选择；不自行选择，也不另建同名项目。用户已给项目 id 时直接核对该项目。
...
项目或任务创建命令超时、断连或返回不完整时，不立即重试。先用 CLI 的只读查询核对刚才提交的项目名，或目标项目中的任务标题和描述；查询命令不确定时查看 CLI 帮助。找到唯一且能确认是本次创建的记录时复用其 id；确认未创建时才重试。无法确定或出现多个候选时，说明候选信息并请求用户判断，避免重复创建。
