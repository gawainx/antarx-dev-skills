...
description: 当用户明确要求初始化 antarx-harness 文档体系或本技能的 Codex 持续开发工作流骨架时使用。泛泛的项目初始化、目录创建或未明确指向本工作流的 /init 不触发；明确要求轻量模式时使用轻量模板。
---

# Codex 持续开发工作流初始化

这个技能只初始化持续开发工作流骨架，不执行具体需求开发。先从用户请求或已有对话确认其选择了本体系；普通应用脚手架、Git 初始化或创建目录不自动升级为 harness 初始化。
...
- 仅在用户明确选择本技能的工作流骨架时执行；已明确选择后，未指定轻量模式则使用全量模板。
