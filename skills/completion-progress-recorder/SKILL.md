...
description: "在用户提及“验收通过”“测试通过”“总结变更”“提交更改”“提交改动”“保持工作区干净”“清理工作区”“收尾记录”等表示当前需求或开发计划已完成的语义时，执行收尾记录流程：检查或创建 docs/PROGRESS.md、检查并清理 git 工作区（必要时提交）、基于开发计划或提交区间生成约150字简体中文连续段落总结，并以“YYYY.MM.dd|当前HEAD前8位: 总结内容”格式仅追加写入 docs/PROGRESS.md。"
---
...
## 执行流程
1. 检查 `docs/PROGRESS.md` 是否存在。
2. 若存在，读取全文并识别写作风格与最后一条记录中的提交哈希，记为 `hash1`；若不存在，则创建空的 `docs/PROGRESS.md`。
...
6. 获取当天日期（格式 `YYYY.MM.dd`）与当前 `HEAD` 前8位短哈希，按以下格式追加到 `docs/PROGRESS.md`：
...
- 禁止修改或重排 `docs/PROGRESS.md` 任何历史条目，只允许文件末尾追加。
